...
function startGame() {
	...
	const Gameboard = (function() {
		...
		const getGameWinner = () => {
			...
			// If there is a winner, or there is a tie, remove the .isEmpty
			// class from each cell, to stop the user from clicking them.
			// This conditional also makes the reset button visible.
			
			if (winnerMarker || gameTurns == 0 && winnerMarker == undefined) {
				const cells = document.querySelectorAll('.cell')
							cells.forEach((el) => {
								el.classList.remove('isEmpty')
								el.classList.add('isFull')
							})
							
				resetBtn.classList.add('isVisible')
			}
	...
	const displayController = (function() {
		...
		const addMarker = () => {
			...
			const showPlayerTurn = () => {
				if (marker == 'X') {
					heading.textContent = `${player1.name}'s turn.`
				} else {
					heading.textContent = `${player2.name}'s turn.`
				}
			}
			...
			cells.forEach((cell) => {
				...
				cell.addEventListener('click', () => {
					if (cell.classList.contains('isEmpty')) {
	...
	const player1 = createPlayer(prompt('What\'s the player 1 name ?'), 'X')
	const player2 = createPlayer(prompt('What\'s the player 2 name ?'), '0')
...
function resetGame() {
	...
	resetBtn.classList.remove('isVisible')
	container.classList.remove('isWinningCombination')
